...
export default function LoginForm1() {
  ...
  async function onSubmit() {//handle login
    ...
    try {
      const response = await AuthMethods.AdminLogIn(username, password, rememberMe);
      console.log(response);
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-transparent mx-4">
      ...
      {showPopUp && <SuccessDialog iconType="success" message="Success" subMessage="You have successfully logged in" showProgressBar={true} redirect="/admin/dashboard"/>}
    </div>
  );
}
